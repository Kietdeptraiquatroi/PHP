...
class DangKyController extends Controller
{
    ...
    function themMoi(DangKyRequest $req){
     ...
     function layDanhSach(Request $request)
    {
    $value = $request->session()->get('username');
    $dsTaiKhoan = TaiKhoan::where('username',$value )->get();
    return view('information',compact('dsTaiKhoan'));
    }
    ...
    function capNhat(Request $req){
       ...
       if($req->password_cu!=NULL)
       {
        ...
        $taiKhoan->save();
        return redirect()->route('ds_tai_khoan_cn');
    }
}
